...
* **Composition / chaining.** `squidmip._recipe.RecipeChain` documents `mip + decon(sigma=2.0)` and
  ordering semantics, and **nothing executes it**. One operator per run. If you need two steps,
  they are two operators and two runs, or one operator that does both.
