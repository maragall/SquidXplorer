...
WHAT THIS FILE IS
-----------------
One function, ``register()``, named by the ``squidmip.operators`` entry point in
``pyproject.toml``. SquidXplorer calls it once, during ``import squidmip``, after its own
...
from squidmip_operator_template._stdev import (
    DEFAULT_DDOF,
    DEFAULT_SMOOTH_SIGMA,
    stdev_op,
)

# NOTE THE IMPORT THAT IS NOT HERE. `from squidmip import Param, add_projector` at MODULE scope
# looks natural and deadlocks: SquidXplorer loads this plugin from inside `import squidmip`, so a
# module-scope import of squidmip is re-entrant. Whichever import happens first, the other sees a
...
#
# which is loud and named, and still a wasted afternoon. Import squidmip INSIDE `register()`. By
# the time it is called, everything a plugin needs is bound. Your own algorithm module
# (`_stdev.py`) should not import squidmip at all — it is plain numpy, which is also what makes it
...
def register() -> None:
    """Register this package's operators. Called by SquidXplorer during ``import squidmip``.
    ...
    ``consumes``
        ...
        when ``squidmip.plane_op`` stamped it; declared explicitly here because this is a z-reducer
    ...
    """
    from squidmip import Param, add_projector      # INSIDE the function — see the note above
