...
Switches, both directions::
    ...
    SQUIDXPLORER_DECON_DEVICE=cpu     # no GPU at all
    SQUIDXPLORER_DECON_DEVICE=mps     # force the GPU, past the guard
    SQUIDXPLORER_DECON_PAD=off        # no transform padding, raw lengths again
    SQUIDXPLORER_DECON_PAD_CPU=1      # opt the petakit CPU path into padding too (see below)
...
#: Device override, same spelling convention as ``SQUIDXPLORER_CACHE_MB`` (``_budget.py:56``) and
#: ``SQUIDXPLORER_PLATE_CACHE`` (``_platecache.py:196``). Values:
#:
...
#: ``SQUIDXPLORER_DECON_PAD=off`` runs every transform at the raw image length again.
...
def _torch_device() -> Optional[str]:
    ...
    torch is NOT a declared dependency of squidxplorer (``pyproject.toml`` has no torch line); it
...
def select_device(shape, *, gpu: bool = True, psf_shape=None) -> Optional[str]:
    ...
    if override in {"mps", "cuda"}:
        ...
        # SQUIDXPLORER_DECON_DEVICE=mps on a CUDA box must fall back, not raise from inside torch
