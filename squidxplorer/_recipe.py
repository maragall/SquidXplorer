...
What the cache STORES is :class:`squidxplorer._result.Result` (Task 2, 2026-07-29): a result that
...
arrays are refused. See :mod:`squidxplorer._result` for why comparison is the wrong shape.
...
:func:`squidxplorer._compose.compose_operator` turns a parsed chain into ONE
:class:`squidxplorer._engine.Operator` whose ``consumes``/``produces``/``params``/``requires`` are
...
``ChainCensus`` grouped cache entries by chain for a plate legend (``squidxplorer/_legend.py``) that was
...
class Recipe:
    ...
    def label(self) -> str:
        ...
        one transform is the drift the naming law (see :mod:`squidxplorer._address`) exists to stop, and
...
class RecipeChain:
    ...
    :func:`squidxplorer._compose.compose_operator` turns the result into one registry-shaped operator.
    ...
    def parse(text: str) -> "RecipeChain":
        ...
        :mod:`squidxplorer._address`) exists to stop -- and it would put a legend row and a runnable
...
class ResultCache:
    ...
    THE VALUE IS A :class:`squidxplorer._result.Result`, AND ONLY THAT (Task 2, 2026-07-29). This store
    ...
    string. Moving it onto :class:`squidxplorer._address.Extent` is Task 3.
    """
...
# Python (no Qt, no numpy) and importing it must stay free, which `squidxplorer._plate` is not.
