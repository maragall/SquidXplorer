...
The public surface is intentionally tiny::
    ...
    from squidxplorer import open_reader, select_fovs, project_well
...
#
# An explicit QT_API in the environment always wins, so `QT_API=pyqt5 squidxplorer-view` remains the
...
# package declaring a `squidxplorer.operators` entry point gets its `register()` run, right here, and
...
#
# LOUD on a broken plugin: this raises out of `import squidxplorer`, naming the plugin and how to
# uninstall it. See `squidxplorer/_plugins.py` for why that is the choice, and for
# `SQUIDXPLORER_NO_PLUGINS=1`, the escape hatch when a plugin stops the app from starting.
#
