...
#: :func:`squidxplorer._napari_view.colormap_hue_rgb`.
...
class RegionViewer(QMainWindow):
    ...
    #: The :class:`squidxplorer._measure.WindowOpen` clock for THIS window's open, set by
    ...
    def _build_top_row(self) -> QWidget:
        ...
        # machine (all n_t=1) — see the rationale in `squidxplorer/_video.py`'s docstring.
    ...
    # a per-window stack -- ``raw`` plus one group per operator -- and ``squidxplorer._layer_tree``
    ...
    # PRODUCER, exactly as ``squidxplorer._op_result`` says it was missing on the plate's side.
    ...
    def _show_progress(self, percent, text: str = "working…") -> None:
        ...
        picture for "running, denominator unknown" — see ``squidxplorer._activity``: a progress bar
    ...
    def deliver_result(self, op: str, result, *, visible: bool) -> int:
        ...
        :class:`~squidxplorer._placement.PlacedArray` carrying the :class:`~squidxplorer._placement.Placement`
    ...
    def address(self):
        ...
        :class:`~squidxplorer._address.Extent`. Everything else answers with an ``Address`` naming the
...
class ViewerManager(QObject):
    ...
    # WHATEVER WORK IS RUNNING, as one immutable ``squidxplorer._progress.ProgressReport``, or None when
...
class OpenViewList(QWidget):
    ...
    def take_status_row(self) -> tuple:
        ...
        :meth:`squidxplorer._logpanel.LogPanel.adopt_status_row` takes them. Callable once; calling it
    ...
    def _on_run_progress(self, report) -> None:
        ...
        rule ``_progress.ProgressReport.percent`` and ``squidxplorer._activity`` already follow, and for
