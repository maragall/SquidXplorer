...
# (After ``consumes``, ``produces`` and ``params``; see :class:`squidxplorer._engine.Operator`.)
#
...
def labels_op(fn: Callable[[Iterable[np.ndarray]], np.ndarray]) -> Callable[..., np.ndarray]:
    ...
    :func:`squidxplorer.add_projector` infers it and every registration of that callable — the shipped
...
def plane_op(fn: Callable[[np.ndarray], np.ndarray]) -> Callable[[Iterable[np.ndarray]], np.ndarray]:
    ...
    ``consumes = PLANE_OP`` on the result so :func:`squidxplorer.add_projector` infers the declaration::
...
def project_well(
    ...
    consumes:
        ...
        is byte-for-byte unchanged. :func:`squidxplorer.add_projector` passes the registry's
        declaration through :func:`squidxplorer.project_plate`.
    ...
    z:
        ...
        z-outer streaming loop in :func:`squidxplorer._stitch.stitch_region`) can pull one plane's
...
def scope_wells(metadata: dict, n_fovs: Optional[int], regions) -> "dict[str, list[int]]":
    ...
    THE ONE RESOLVER, shared by :func:`squidxplorer.project_plate` and :func:`squidxplorer.stitch_plate`,
