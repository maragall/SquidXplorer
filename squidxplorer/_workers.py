...
Gap 6 of the GUI backlog plan (2026-07-29), step 2 of the split of ``squidxplorer/_viewer.py``.
...
:mod:`squidxplorer._plate_overview` beside their only caller. They are the reason the arrows point the
...
exactly as before. ``squidxplorer._region_viewer`` also imports ``_SpotWorker``, ``_FocusWorker`` and
...
#: and ``SQUIDXPLORER_PLATE_CACHE=0`` need to be able to say.
_CACHE_AUTO = object()
...
class _OperatorWorker(QThread):
    ...
    # region operator), carried as one immutable ``squidxplorer._progress.ProgressReport``. A separate
    ...
    def run(self):
        ...
        # RUN's duration, not to a thread (see squidxplorer._logpane.capture_stdout_to_log).
...
class _MinervaRenderWorker(QThread):
    ...
    :func:`squidxplorer._minerva.render_exhibit`.
...
class _PreviewWorker(QThread):
    ...
    # ``squidxplorer._progress.ProgressReport`` per completed unit. Julio asked for one bar covering
    ...
    def __init__(self, reader, meta, fov_index: dict, order: list, mosaic: bool = True,
        ...
        # `_path`, an unwritable cache dir, SQUIDXPLORER_PLATE_CACHE=0 -- so this worker never has to
    ...
    def run(self):
        ...
        # ``squidxplorer._logpane.capture_stdout_to_log``). Julio: "when I run a preview it doesn't show
