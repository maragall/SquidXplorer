...
:class:`~squidxplorer._napari_view.MosaicLayers`, and there was no before/after toggle for any of
them. That is not a bgsub bug; bgsub is just where it was noticed. The group toggle UI
(:mod:`squidxplorer._layer_tree`) was already built, already tested and already mounted -- it had
...
----------------------------------
The mosaic is built with :func:`squidxplorer._mosaic_source.fuse_region_mosaic` and
:func:`squidxplorer._placement.fov_offsets_px` -- **the raw mosaic's own placement helpers**, not
...
class OperatorResult:
    ...
    ``layer.metadata["squidxplorer"]["op"]`` (see :class:`squidxplorer._napari_view.MosaicKey`), and
...
class RegionResultAccumulator:
    ...
    def add(self, fov: int, planes: Any) -> None:
        ...
        :class:`squidxplorer._placement.PlacedArray`, which carries the :class:`Placement` that
    ...
    def _bbox(self):
        ...
        A region operator's pixels arrive carrying their own :class:`~squidxplorer._placement.Placement`
