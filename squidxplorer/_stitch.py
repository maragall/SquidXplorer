...
sidecar shadowing it). It is one table again as of 2026-08-05: ``squidxplorer.add_region_operator``
...
def estimate_region_flatfield(
    ...
    *,
    ...
    ``estimate_flatfield_channel`` per channel. squidxplorer already wraps that exact estimator as
    :func:`squidxplorer.estimate_profile`, so the algorithm is Julio's BaSiC port either way.
    ...
    # The plate bar cannot cover it either: its unit is the REGION (squidxplorer._progress.unit_plan),
...
def _selected_profiles(names: Sequence[str]) -> Optional[dict]:
    ...
    THE ONE OWNER OF "the profile the user chose". ``squidxplorer._flatfield``'s module global
...
class _FlatfieldReader:
    ...
    fuses every z independently. squidxplorer DOES z-reduce, so correcting the projector's OUTPUT
...
class _SeamSource:
    ...
    squidxplorer runs tilefusion's pieces on IN-MEMORY arrays (see the module docstring), so there
...
def stitch_region(
    ...
    **The output side is spilled, not swapped.** A 10-plane 4-channel mosaic of that well is
    8.79 GB, on a 16 GB machine that is also holding the tiles. :func:`squidxplorer._volume.allocate`
    backs it with a scratch file above 2 GB and :func:`squidxplorer._volume.release` drops the
    ...
    # stage unconditionally, so squidxplorer was the only one of the two NOT correcting distortion
    # unless asked.
    #
    ...
    # Spilled to a scratch file above 2 GB (see squidxplorer._volume): a 10-plane 4-channel mosaic of
...
def _accepts_kwarg(fn, name: str) -> bool:
    ...
    operator table is an EXTENSION POINT (:func:`squidxplorer.add_region_operator`) — a third-party
...
def stitch_plate(
    ...
    **operator_kwargs,
    ...
    The region-operator twin of :func:`squidxplorer.project_plate`, and deliberately the SAME
    ...
    n_fovs:
        ...
        straight to :func:`squidxplorer.select_fovs`.
    workers:
