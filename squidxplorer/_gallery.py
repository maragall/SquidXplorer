...
(:mod:`squidxplorer._gallery_window`) holds nothing but layout.

ADOPTING hongquanli/gallery-view's RECIPE, NOT IMPORTING IT
-----------------------------------------------------------
Same precedent as :mod:`squidxplorer._napari3d` (read its docstring): gallery-view pins napari <0.6
...
* **The contrast function is ours, not gallery-view's 0.5/99.5 percentiles.**
  :func:`squidxplorer._contrast.auto_contrast` is a port of ``maragall/stitcher``'s window (background
...
* **The windowing is :func:`squidxplorer._montage.composite`, not a private ``mip_to_rgba``.**
...
``window`` as DATA, computed in :class:`squidxplorer._gallery_window.GalleryWorker` beside the pixels
...
class GalleryCell:
    ...
    ``image`` is native dtype (uint8/uint16) because :func:`squidxplorer._montage.composite` windows
    ...
    ``window`` is ``None`` when :func:`squidxplorer._contrast.auto_contrast` refused, which it does for
...
def plan_cell(meta: Mapping, region: str, fovs: Sequence[int],
    ...
    Pure geometry over :func:`squidxplorer._placement.fov_offsets_px`, which normalises the top-left
