...
channel is a thing the loop knows. See ``squidxplorer.projection.bind_channel`` and
...
a contributor reads, and ``squidxplorer/_plugins.py`` is how an operator in SOMEBODY ELSE'S package
reaches this table without editing ``squidxplorer/__init__.py``.

COMPOSITION (2026-08-05) is the same rule ONE more time, and it is why the four declarations above
had to be declarations. ``squidxplorer._compose`` builds ONE ``Operator`` out of several and derives its
...
class MissingOperatorDependency(MissingDependency):
    ...
    The exact twin of :class:`squidxplorer._spots.MissingSegmenterDependency`, and deliberately the
...
class Operator:
    ...
    :data:`squidxplorer.projection.INTENSITY` / :data:`squidxplorer.projection.LABELS`. Default
    ...
    :class:`squidxplorer._spots.Segmenter` has had it since Cellpose landed: the importable module
    ...
    def with_params(self, operator_kwargs: Optional[dict] = None) -> Projector:
        ...
        Split out for composition (:mod:`squidxplorer._compose`), which builds a chain's steps before
...
def add_projector(name: str, projector: Projector, *, consumes=None, produces=None,
    ...
    projector:
        A callable with the :func:`squidxplorer.project` signature — takes an iterable of
        ...
        ``plane -> plane`` function is lifted into this shape by :func:`squidxplorer.plane_op`.
    consumes:
        ...
        ``consumes`` attribute (:func:`squidxplorer.plane_op` stamps one) and otherwise falls back to
    ...
    produces:
        ...
        :data:`squidxplorer.projection.LABELS`. ``None`` (default) reads the callable's own
        ``produces`` attribute (:func:`squidxplorer.projection.labels_op` stamps one) and otherwise
...
def add_region_operator(name: str, operator, *, produces=None, params: Sequence[Param] = (),
    ...
    declared anything else would be run by the wrong loop — and :func:`squidxplorer.stitch_plate`
...
def _declare(name: str, fn, *, consumes, produces, params, requires, region: bool) -> None:
    ...
    *region* is not a stored field — it is the difference between the two callable shapes, and it
    ...
    :data:`~squidxplorer.projection.REGION_OP`.
    """
...
def operator_available(name: str) -> tuple[bool, str]:
    ...
    The twin of :func:`squidxplorer._spots.segmenter_available`, for the operator table. Read this
...
def _resolve_operator(name) -> Operator:
    ...
    :func:`squidxplorer._output.write_plate`, :func:`squidxplorer._stitch.stitch_region`, the CLI, the
    ...
    * a CHAIN EXPRESSION (``"flatfield + decon + mip"``, or a bare name with
      parameters) is parsed and composed — see :mod:`squidxplorer._compose` for what it derives and
      which combinations it refuses.
    * a :class:`~squidxplorer._recipe.RecipeChain` is composed directly, for a caller holding the
...
def project_plate(
    ...
    *,
    ...
    n_fovs:
        ...
        IMA-187 mosaic path). Passed straight to :func:`squidxplorer.select_fovs`.
    workers:
