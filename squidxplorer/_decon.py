...
via :func:`squidxplorer.plane_op`, so ``project_well``'s existing group-by-then-reduce loop hands
...
* **``petakit.deconvolve(method="rl")``** — PetaKit5D's ``decon_lucy_function.m`` ported, with
  ...
  CuPy is CUDA, so "GPU when present" has always meant "never, on a Mac". :mod:`squidxplorer._decon_gpu`
  ...
  opt-in (``SQUIDXPLORER_DECON_PAD_CPU=1``) because petakit's Biggs-Andrews lambda is reduced over
...
format and re-parsing the metadata of a folder squidxplorer had already opened. It detects
...
from squidxplorer's own read of the same folder:
...
def emission_um_for(channel) -> float:
    ...
    1. ``_channels.excitation_nm`` reads the excitation line off the channel name — squidxplorer's
...
class OpticsParams:
    ...
    def from_acquisition(cls, path, channel: str) -> "OpticsParams":
        ...
        Every field comes from SQUIDXPLORER'S OWN read of the acquisition (module docstring): the
...
def _run(volume: np.ndarray, psf: np.ndarray, iterations: int, gpu: bool) -> np.ndarray:
    ...
    measured. :mod:`squidxplorer._decon_gpu` runs the identical Biggs-Andrews RL update on Metal
    ...
    ``SQUIDXPLORER_DECON_DEVICE`` overrides the choice in both directions.
    ...
    :func:`squidxplorer._decon_gpu.rl`, where it can also restrict the Biggs-Andrews reduction to
    ...
    ``SQUIDXPLORER_DECON_PAD_CPU=1`` and is off here by default.
    """
...
def optics_for_channel(path, channel: str) -> OpticsParams:
    ...
    2. the acquisition's own metadata for *this channel* (squidxplorer's parse, memoised);
...
def decon_op(
    ...
    :func:`squidxplorer.plane_op`), so the registry infers the declaration and z survives.

    PER-CHANNEL OPTICS. With ``optics=None`` the returned callable also carries ``for_channel``,
    the declaration :func:`squidxplorer.projection.bind_channel` reads: ``project_well`` calls it once
