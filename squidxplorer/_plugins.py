...
edit. It was impossible *outside* it. ``squidxplorer/__init__.py`` ends in a hardcoded side-effect
import list (``from squidxplorer import _background, _decon, _flatfield, _spots``), and that list is
...
and on ``import squidxplorer`` we import ``my_package``, call its ``register()``, and its operators are
...
* **A scan of a plugin directory** (``~/.squidxplorer/operators/*.py``) — needs its own installer, its
...
resolve, aborts ``import squidxplorer`` with an :class:`OperatorPluginError` naming the plugin, its
...
The escape hatch is an environment variable, not a code path: ``SQUIDXPLORER_NO_PLUGINS=1`` skips
...
class OperatorPluginError(RuntimeError):
    ...
    Raised out of ``import squidxplorer``. See this module's docstring for why that is loud rather
    than skipped, and for the ``SQUIDXPLORER_NO_PLUGINS=1`` escape hatch.
    """
...
def load_operator_plugins(group: str = GROUP) -> list[str]:
    ...
    Called once, from the bottom of ``squidxplorer/__init__.py``, after the built-in registrations.
