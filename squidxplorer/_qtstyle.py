...
into ``squidxplorer._viewer`` at call time to borrow three of them::

    from squidxplorer._viewer import _BTN_QSS, _CHECK_QSS, _COMBO_QSS   # inside a function body
...
def operator_card(label: str, blurb: str):
    ...
    ELIDED WITH A TOOLTIP, not shortened at the registry. The blurbs in ``squidxplorer._operations``
