...
class BrickedVolume:
    ...
    def _frame_camera(self) -> None:
        ...
        user has resized, it either clips the ROI or opens further out than asked, and it reads
        as somebody having preferred a different zoom. One rule, one place, and a test on a
        deliberately non-square canvas.

        The margin stays this view's own 0.10 rather than ``FRAME_MARGIN``: a bricked volume
        culls bricks outside the frustum, and the wider margin is what keeps the ROI's own edge
        bricks resident. That is a 3D loading decision, not a framing convention.
        """
