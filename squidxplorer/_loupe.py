...
the napari canvas in a view window (``squidxplorer._napari_loupe``), raised with shift-left-click.
...
from squidxplorer._montage import _area_downsample, _pct_window
from squidxplorer._mosaic_source import MemoryBoundedLRUCache
# THE LAYOUT CONTRACT, not a hand-parse. `_ZarrLoupeSource` asks `squidxplorer/contract` where a
...
# The same quarter-share discipline as the plate's other consumers — the loupe's crop and
# coarse-plane caches and the contrast-window memo. Each is an independent LRU bounded at this
# cap; the cap is a ceiling, not a reservation. (Moved here with the engine on 2026-08-16; the
# plate's tile QImage cache keeps its own copy of the cap in `_plate_overview`.)
...
class _RawLoupeSource(_LoupeSource):
    ...
    def coarse(self, well_id, time_point: int = 0):
        ...
        if hit is None:
            ...
            hit = np.stack([_area_downsample(p[c], _LOUPE_COARSE_PX, _LOUPE_COARSE_PX)
...
# THE OWNERSHIP RULE FOR THE WORKER BELOW is `squidxplorer._qthread_life.detach`, not anything in
