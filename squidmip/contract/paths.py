...
gate in :mod:`squidmip.contract.version` worth building: a declared version is decorative unless
...
def field_path(base, wellpath, fov, level: Optional[object] = None) -> str:
    ...
    chooses. SquidMIP writes ``"0"``, ``"1"``, ..., but a conforming store may write anything, so a
