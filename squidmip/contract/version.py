...
* ``PLATE_CONTRACT_VERSION`` below is SQUIDMIP's promise about the store: the group hierarchy, the
...
WHERE THE STAMP LIVES. ``zarr.json -> attributes -> squidmip -> plate_contract_version`` on the
...
#: The version of the SquidMIP plate contract that this build writes and understands.
#:
...
def compare_contract_version(declared: Optional[str], supported: str = PLATE_CONTRACT_VERSION):
    ...
    if got_major != want_major:
        raise PlateContractError(
            f"this store declares plate contract version {declared}, but this build of SquidMIP "
            ...
            "downgrade SquidMIP to a build whose major matches, or re-run the projection. See "
