...
The replacement is this module. A result carries its own :class:`~squidmip._address.Extent` (WHERE
...
----------------------------------------------------------------
:class:`~squidmip._address.Extent` already has a ``channels`` field, and it looks like a duplicate.
...
:meth:`squidmip._op_result.OperatorResult.plane`.

ON THE WORD "SHAPE"
-------------------
The plan calls this second half "its shape". The word is spoken for three times over in this
codebase: ``ndarray.shape`` is an axis tuple, :mod:`squidmip._plate_shape` is plate geometry, and
the writer's validators use it in the numpy sense. Rule 2 of the naming law (see
:mod:`squidmip._address`) says a software concept takes a word that cannot collide, so this one is
...
------------------------------
The cache KEY is untouched: :class:`squidmip._recipe.ResultCache` still keys
``(scope, version, chain)`` with ``scope`` the packed ``row*1e6 + col*1e4 + roi`` string from
``_plate.cache_scope``. Migrating the key onto :class:`~squidmip._address.Extent` is Task 3 work,
...
This module is pure Python: no Qt, no numpy, like :mod:`squidmip._address` and
:mod:`squidmip._recipe`. It is the model, testable in isolation, and importing it must not cost a
consumer anything.
"""
...
class Substance:
    ...
    the difference between this and an :class:`~squidmip._address.Extent`: an extent may describe a
...
class Result:
    """A cached result: WHERE it is (:class:`~squidmip._address.Extent`), WHAT it is made of
    ...
    :class:`squidmip._op_result.OperatorResult`. It may be ``None``: a result read back from a
    ...
    def to_dict(self) -> dict:
        ...
        Pixels go to disk as pixels (see :mod:`squidmip._platecache`); what has to travel as JSON
...
def composite_plate(cells: "Mapping[Any, Result]", channel: str) -> dict:
    ...
    :class:`~squidmip._address.Address`, a ``(row, col)``, a scope string) survives. A cell that
