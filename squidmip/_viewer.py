...
class _MosaicWorker(QThread):
    ...
    ready = pyqtSignal(str, str, object, object, object)   # region, channel, data, bbox_um|None,
    #                                                       contrast window|None
    ...
    def run(self):
        ...
        for ch in self._channels:
            ...
            if res is None:
                ...
                continue
            data, _step, _nz = res
            self.ready.emit(self._region, ch, data, bbox, self._window(data))
            n += 1
        self.finished_count.emit(n)

    def _window(self, data):
        """napari's OWN autoscale, over ONE plane, computed HERE instead of on the GUI thread.

        MEASURED, which is why this exists. Handing napari a lazy ``(z, y, x)`` stack with no
        ``contrast_limits`` makes it call ``calc_data_range``, which samples the BOTTOM, MIDDLE
        and TOP planes — three fused planes per channel, on the GUI thread, inside ``add_image``.
        On the 10x tissue set that measured 941 ms per channel of a 3.2 s region change; passing
        a window instead brought ``add_image`` from 663 ms to 173 ms per channel.

        This does NOT introduce a second contrast rule, which is the thing 19cd491 and IMA-261
        were careful to avoid. It is literally ``napari.layers.utils.layer_utils.calc_data_range``
        — napari's function, napari's min/max policy — given the plane the user is about to look
        at rather than three planes they are not. napari still OWNS contrast: the user retunes on
        napari's own slider, ``add_mosaic`` writes this inside ``programmatic()`` so the plate's
        sink cannot mistake it for a user gesture, and the plate still only ever follows.

        Returns None rather than guessing if napari is not importable; ``add_mosaic`` then
        behaves exactly as before and napari autoscales itself.
        """
        try:
            from napari.layers.utils.layer_utils import calc_data_range
        except Exception:                           # noqa: BLE001 - ndv fallback has no napari
            return None
        if not hasattr(data, "compute"):
            return None                             # already a plain plane; napari's scan is cheap
        z = max(0, min(self._z_index, data.shape[0] - 1))
        try:
            plane = np.asarray(data[z])
        except Exception as exc:                    # noqa: BLE001 - reported, never swallowed
            self.problem.emit(
                f"{self._region}: could not read z={z} to set the contrast window "
                f"({type(exc).__name__}: {exc}); napari will autoscale instead.")
            return None
        lo, hi = calc_data_range(plane, rgb=False)
        return (float(lo), float(hi))
...
class PlateWindow(QMainWindow):
    ...
    def ingest(self, path: str):
        ...
        self._readout.setText(
            f"{len(self._fov_index)} regions · slide the region slider or double-click to "
            f"open one{note}")
    ...
    def _on_mosaic_plane(self, op: str, region: str, channel: str, plane, bbox_um,
        ...
        # `window` is NAPARI'S OWN autoscale (`calc_data_range`), run over the visible plane on
        # the WORKER thread — see `_MosaicWorker._window` for the measurement that motivated it.
        # napari still owns contrast: same function, same min/max policy, same layer slider, and
        # `add_mosaic` writes it inside `programmatic()` so the plate's sink cannot read it as a
        # user gesture. What changed is WHERE and over how much data it is computed, not WHOSE
        # rule it is. `None` (ndv fallback, or a plane that could not be read) falls straight
        # back to napari autoscaling for itself.
        #
        ...
        # and using napari's feature means using napari's FUNCTION, not letting napari's call site
        # pick the data. Left to itself over a lazy z-stack it samples three fused planes on the
        # GUI thread; the user retunes on the layer's own contrast slider either way, and that
        # slider is still the single owner the plate follows.
