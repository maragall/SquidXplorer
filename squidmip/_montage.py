...
def _pct_window(a: np.ndarray, pct=_PCT) -> tuple[float, float]:
    ...
    Widening it to ``(lo, lo + 1)`` is a real bug wearing a helpful face — ``+1`` is one DATA unit,
    so ``(v - lo) / 1`` clips to 1.0 and a blank or saturated channel renders FULL WHITE and reads
    as signal. The loupe carried a private copy that DID widen, and that is the defect IMA-242
    collapsed (see the note in ``_plate_overview``).
    ...
    :func:`_area_downsample` the one resampler. Three modules BELOW the GUI boundary already cite
    this as the rule they must not duplicate (``_mosaic_source``, ``_napari_view``, ``_contrast``),
    and a rule they can only reach by importing a QWidget module is a rule they will copy.
    """
    if a.size == 0:
        return 0.0, 0.0
