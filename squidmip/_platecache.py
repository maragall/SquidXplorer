...
blocker. Verified at HEAD: ``grep -rn 'platformdirs|user_cache_dir' squidmip/`` returned nothing,
...
this cache is the producer and :class:`squidmip._tilesource.CompositePlateSource` is the consumer.
...
ENV_ENABLED = "SQUIDMIP_PLATE_CACHE"
...
def cache_root() -> Path:
    ...
    return Path(platformdirs.user_cache_dir("squidmip", "cephla"))
...
def plate_token(path, extra: Sequence = ()) -> str:
    ...
    *extra* carries everything about the cached RENDERING that must invalidate independently of
    ...
    """
    h = hashlib.sha256()
    h.update(f"squidmip-plate-cells v{FORMAT_VERSION}\0".encode("utf-8"))
