...
(The flag keeps its name for compatibility; it selects any OPERATOR, not only a z-reduction.)
    ...
    squidmip <acquisition> --projector stitch       # a REGION operator, same flag
    squidmip <acquisition> --wells B2,B3            # named subset
    squidmip <acquisition> --projector cellpose --param min_area_px=80
    squidmip <acquisition> --tiff                   # also write the per-plane TIFF export
...
an operator installed from another package through the ``squidmip.operators`` entry-point group
...
-----------------------------------------------
``for d in */; do squidmip "$d" || echo "FAILED: $d"; done`` has to work, so "the run finished"
...
class ProcessParameters(BaseModel, use_attribute_docstrings=True):
    ...
    # The table in this help text is BUILT from the registry at import time (after squidmip's
...
def run(params: ProcessParameters, *, stop=None) -> dict:
    ...
    # SQUIDMIP_WELLPLATE_FORMAT (documented there as the headless/CLI override) -> inferred from
...
def exit_code(manifest: dict) -> int:
    ...
    a plate exists but is short of what was asked for, so ``squidmip d || echo FAILED`` catches
...
def interrupt_stop():
    ...
    drains its in-flight writes and the store keeps its ``.squidmip-incomplete`` marker, which is
...
def main(args: Optional[list[str]] = None) -> int:
    ...
    whole fix for a batch loop: ``for d in */; do squidmip "$d" || note "$d"; done`` could not
    ...
    try:
        ...
        for err in exc.errors():
            ...
            print(f"squidmip: {where}: {err['msg']}", file=sys.stderr)
        return EXIT_USAGE
