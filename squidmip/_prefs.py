...
This is deliberately NOT :class:`squidmip._region_viewer.ViewSettings`, and the distinction is the
...
#: The one file. ``SQUIDMIP_PREFS`` overrides it, which is what makes this testable without
...
def prefs_path() -> Path:
    """Where preferences live. ``~/.squidmip/prefs.json`` unless ``SQUIDMIP_PREFS`` says otherwise.

    ``~/.squidmip`` is the directory ``_plugins.py`` already names for user-level squidmip state,
    so this adds a file to an existing home rather than a second dot-directory.
    """
    override = os.environ.get(_ENV)
    if override:
        return Path(override)
    return Path.home() / ".squidmip" / "prefs.json"
