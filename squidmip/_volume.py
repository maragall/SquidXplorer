...
The z-outer loop in :func:`squidmip._stitch.stitch_region` bounds the INPUT side (one z of tiles
...
temp directory (``SQUIDMIP_SCRATCH_DIR`` overrides), is unlinked the moment it is mapped on
...
_SPILL_ENV = "SQUIDMIP_SPILL_BYTES"
_SCRATCH_ENV = "SQUIDMIP_SCRATCH_DIR"
...
def allocate(shape: Sequence[int], dtype, *, threshold: Optional[int] = None,
    ...
    fd, path = tempfile.mkstemp(prefix="squidmip-fuse-", suffix=".bin", dir=str(directory))
    try:
        handle = open(fd, "w+b")
