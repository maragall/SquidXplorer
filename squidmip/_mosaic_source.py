...
def mosaic_fov_bboxes_um(meta: dict, region: str) -> "dict[int, tuple[float, float, float, float]]":
    ...
    the region's recorded positions and each offset is :func:`~squidmip._placement.fov_offsets_px`
    -- exactly what :func:`mosaic_bbox_um` above uses, and therefore exactly what
    ``_workers._MosaicWorker`` placed this window's layer with. So a box this returns lands on the
    pixels it names, in napari's world, with no conversion.
    ...
    translation); everything on the mosaic path treats it as the frame's TOP-LEFT. The two are half
    a frame apart -- measured, 195.9 um on the 40x AF-sweep set -- and half a frame of UNIFORM
    shear renders as a perfectly plausible picture of the wrong tissue. Each convention is right
    for its own surface (that one places the PLATE, this one places a WINDOW's mosaic), so neither
    is a bug; picking the wrong one HERE would be. The check that catches it is free and already on
    screen: the centre of every box this returns must satisfy ``fov_at_point`` -> that same fov.
    ...
    p = float(pixel_size)
    # The region origin, spelled exactly as `mosaic_bbox_um` spells it. Two spellings of one origin
    # is how the boxes and the mosaic they sit on drift apart by a sub-pixel nobody notices until
    # it is a whole frame.
...
def fovs_overlapping_bbox(meta: dict, region: str,
    ...
    ONE GEOMETRY, and since 2026-08-13 it is one FUNCTION: the boxes come from
    :func:`mosaic_fov_bboxes_um` above, which is the same placement rule that drew the pixels the
    user boxed. This used to build them inline, and so did :func:`fov_at_point` below -- two copies
    of "where is FOV 7" sitting in one file under a docstring warning against exactly that. A third
    consumer (the FOV walk) was what made the cost of keeping them visible.
    ...
    statement as "run on nothing". That is why the raise from ``mosaic_fov_bboxes_um`` is caught
    here and nowhere else -- see its docstring for why the two contracts differ.
    """
    if bbox_um is None:
        return None
    try:
