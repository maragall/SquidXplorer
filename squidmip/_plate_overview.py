...
from squidmip._montage import _PCT, _area_downsample, _pct_window, composite  # noqa: F401
from squidmip._qthread_life import _DETACHED, detach as _detach  # noqa: F401
from squidmip._loupe import (  # noqa: F401 (re-exports: the engine moved, the spellings did not)
    loupe_inset_rect,
    loupe_label,
    paint_loupe_inset,
    _LOUPE_PX,
...
# `_loupe.paint_loupe_inset` now, which carries its own `_INSET_SCALE_PX`. Kept here it would be a
# constant nothing reads — the `_voxel_scale` shape CLAUDE.md is written against.
...
# `_PCT` and `_pct_window` MOVED to `squidmip._montage` (2026-08-13), where `composite` and
# `_area_downsample` already live. They are imported back below so every existing spelling --
# `_plate_overview._pct_window`, `_viewer._pct_window` -- keeps working and nothing about the
# plate changes. The move is what lets `squidmip._loupe` reach the one percentile rule without
# importing this 3000-line QWidget module; see that module's docstring.


# THE LOUPE ENGINE MOVED to `squidmip._loupe` (2026-08-13), verbatim, when the napari canvas
# needed a loupe too (`squidmip._napari_loupe`, raised with shift-left-click). What stayed
# HERE is this widget's own half of the gesture: the press-and-hold that arms it
# (`_arm_loupe`), the hit-test that resolves which field it is over (`_loupe_target`), and
# the cursor-avoiding placement inside this widget's paintEvent (`_paint_loupe`). What moved
# is everything both surfaces must agree about. See that module's docstring for why a second
# loupe was not an option -- the IMA-242 note it carries is the record of what one cost.
#
# Imported back below, so `_plate_overview.loupe_scale`, `_viewer.loupe_scale` and every
# existing test spelling keep working unchanged.
...
class PlateOverview(QWidget):
    ...
    def _paint_loupe(self, p: QPainter):
        ...
        ``_loupe.paint_loupe_inset``, shared with the napari canvas loupe, because a second painter
        is how the two insets would come to disagree about what a scale bar means. That is the
        IMA-242 shape, and the note further up this file is the record of what it cost last time.
        """
