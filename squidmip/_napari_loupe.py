...
EVERYTHING ABOUT PIXELS IS ``squidmip._loupe``'s, AND THAT IS THE POINT
-----------------------------------------------------------------------
...
from squidmip._qthread_life import detach
...
#: Where the remembered factor is kept. A magnification is "a decision the user makes ONCE, about
#: the application, and expects to still be true tomorrow" -- ``_prefs``' own words for what it is
#: for. Without this the wheel would be re-taught to every window, which is the opposite of a
#: setting. Deliberately NOT `ViewSettings`: that is per-window look (contrast, channels), and a
#: loupe factor is not a property of one region's picture.
_MAG_PREF = "loupe_mag"


def _default_mag_index() -> int:
    """Where the ladder starts: what the user last chose, else the engine's own default.

    Falls back rather than failing: ``_prefs.get`` never raises, an unreadable prefs file degrades
    to the default, and a value that is no longer on the ladder (someone edited the JSON, or the
    ladder changed) snaps to the nearest rung instead of being honoured as something the wheel
    could never reach again.
    """
    from squidmip import _prefs

    want = _prefs.get(_MAG_PREF, float(_LOUPE_MAG))
    try:
        want = float(want)
...
class CanvasLoupe(QObject):
    ...
    def _on_wheel(self, viewer, event):
        ...
        # REMEMBERED, so the next window and the next session open where the user left off. Write
        # failures are the pref store's problem and it degrades quietly by design; a loupe that
        # refused to magnify because a JSON file was read-only would be absurd.
        try:
            from squidmip import _prefs

            _prefs.set(_MAG_PREF, float(_MAG_LADDER[nxt]))
        except Exception:                 # noqa: BLE001 - a remembered preference, never fatal
            pass
        self._request()
    ...
    def _ensure_worker(self):
        ...
        if self._worker is None:
            from squidmip._loupe import _LoupeWorker
    ...
    def _request(self) -> None:
        from squidmip._mosaic_source import fov_pixel_at_point
