...
def test_a_mosaic_is_not_derivable_without_positions_and_says_so_by_returning_none():
    ...
    assert mosaic_bbox_um(meta, "A1") is None
    meta["n_z"] = 4
...
def test_an_unreadable_fov_is_ANNOUNCED_and_not_only_left_as_a_hole(caplog):
    """The hole stays put, but a black rectangle in a fluorescence mosaic reads as "no signal there" (a fact about the sample) unless something announces it"""
    import logging

    meta = _meta({("A1", 0): (0.0, 0.0), ("A1", 1): (12.0, 0.0)}, [0, 1])
    with caplog.at_level(logging.WARNING):
        mosaic, _ = fuse_region_mosaic(_Reader(fail=[0]), meta, "A1", "488")
    assert mosaic.shape == (4, 12), "the hole must stay put, not shift its neighbours"
    assert np.all(mosaic[:, :6] == 0) and np.all(mosaic[:, 6:] == 2)
...
def test_the_raw_preview_returns_a_pyramid_of_strictly_decreasing_levels():
    """napari's ``multiscale=True`` contract: a LIST, highest resolution first, each level strictly smaller than the one above it in both displayed axes."""
    from squidxplorer._mosaic_source import fuse_region_pyramid

    reader = _StepReader()
    levels, step, nz = fuse_region_pyramid(reader, _pyr_meta(nz=6), "A1", "488")
    ...
    for i, lv in enumerate(levels):
        assert lv.ndim == 3 and lv.shape[0] == 6, f"level {i} lost or changed the z axis: {lv.shape}"
...
def test_fusing_a_level_also_yields_the_coarser_levels_from_the_same_decode():
    """napari pulls TWO levels per channel per z (the visible one, and the coarsest one for the thumbnail); TIFF decode is whole-frame, so the thumbnail"""
    from squidxplorer._mosaic_source import fuse_region_pyramid
...
def test_the_coarsest_level_costs_one_decode_per_fov_and_nothing_finer():
    """Materialising the coarsest rung (the contrast seed, napari's thumbnail pull) must cost exactly one decode per FOV — a finer plane built on the side"""
    from squidxplorer._mosaic_source import fuse_region_pyramid
...
def test_the_coarsest_rung_repull_is_a_plane_cache_hit_even_after_the_frames_evict():
    """napari pulls the coarsest rung per thumbnail; a full-window compute caches the WHOLE plane, so the re-pull is a lookup even once the decoded frames"""
    from squidxplorer._mosaic_source import fuse_region_pyramid
...
def test_a_warm_pan_at_a_coarse_rung_re_reads_nothing_even_when_full_frames_cannot_be_cached():
    """A rung's decimated subframes are cached at their own size, so a revisited window costs zero decodes even where the region's FULL frames outsize the"""
    from squidxplorer._mosaic_source import fuse_region_pyramid
...
def test_every_rung_is_pixel_identical_to_the_reference_fusion():
    """Every windowed rung — fine strides and converted coarse rungs alike — materialises bit-exact to :func:`_fuse_levels` at the same stride: one paste"""
    from squidxplorer import _mosaic_source as ms
...
def test_a_region_whose_every_fov_is_unreadable_is_an_error_not_a_blank_mosaic():
    """A mosaic where EVERY FOV failed is not a picture at all; a black plane would report a read failure as empty tissue."""
    from squidxplorer._mosaic_source import fuse_region_pyramid
...
def test_a_plane_larger_than_the_whole_cache_is_a_loud_error_not_a_silent_no_op():
    """A cache that logs and silently no-ops on an oversized item leaves slowness as the only symptom; this must raise instead."""
...
def test_the_plane_cache_serialises_concurrent_writers():
    """dask may compute planes concurrently, so two threads can land in put() at once."""
    import threading
...
def test_the_pyramid_levels_agree_with_the_full_resolution_picture():
    """A misregistered level looks fine in a shape assertion and wrong on screen: a feature at a given fraction of the mosaic must land at that same"""
    from squidxplorer._mosaic_source import fuse_region_pyramid
...
def test_the_cache_is_keyed_by_the_acquisition_not_by_the_reader_object():
    """Keying on ``id(reader)`` would miss the cache for a second reader over the same acquisition, and (since CPython recycles ids) could serve another"""
    from squidxplorer._mosaic_source import _source_token, fuse_region_pyramid

    with pytest.raises(ValueError, match="_path"):
        _source_token(object())          # a reader with no identity would share every key
    meta = _pyr_meta()
