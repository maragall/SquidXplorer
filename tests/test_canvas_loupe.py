...
from squidmip._loupe import (  # noqa: E402
    _LOUPE_PX,
    canvas_scale,
    capped_at_native,
    loupe_inset_rect,
...
def test_the_wheel_changes_magnification_and_never_zooms_the_camera(qapp):
    ...
    from squidmip._napari_loupe import _MAG_LADDER
...
def test_the_chosen_magnification_is_remembered_across_windows(qapp, tmp_path, monkeypatch):
    """A factor the user picked with the wheel must still be there next time.

    Otherwise the wheel has to be re-taught to every window, which is the opposite of a setting.
    ``~/.squidmip/prefs.json`` is the repo's one home for "a decision made ONCE about the app".
    """
    from napari.utils.interactions import mouse_press_callbacks, mouse_wheel_callbacks

    from squidmip import _prefs
    from squidmip._napari_loupe import _MAG_LADDER, _MAG_PREF, _default_mag_index

    monkeypatch.setenv(_prefs._ENV, str(tmp_path / "prefs.json"))
    ...
    assert _prefs.get(_MAG_PREF, None) == chosen
    assert _MAG_LADDER[_default_mag_index()] == chosen, "the next window must open where this left"


def test_a_nonsense_remembered_factor_snaps_to_the_ladder(qapp, tmp_path, monkeypatch):
    """A hand-edited or stale prefs value must not leave the wheel somewhere it cannot reach."""
    from squidmip import _prefs
    from squidmip._napari_loupe import _MAG_LADDER, _MAG_PREF, _default_mag_index

    monkeypatch.setenv(_prefs._ENV, str(tmp_path / "prefs.json"))

    _prefs.set(_MAG_PREF, 9.3)
    assert _MAG_LADDER[_default_mag_index()] == 8.0, "nearest rung, not a value off the ladder"

    _prefs.set(_MAG_PREF, "not a number")
