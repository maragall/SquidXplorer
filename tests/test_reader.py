...
def test_channels_come_from_filenames_and_colors_from_yaml_then_fallback(squid_dataset):
    root, _ = squid_dataset
    by_name = {c["name"]: c for c in open_reader(root).metadata["channels"]}
    assert set(by_name) == {CH_IN_YAML, CH_NOT_IN_YAML}
...
def test_read_rejects_non_2d_and_uint32(squid_dataset):
    root, _ = squid_dataset
    tifffile.imwrite(root / "0" / f"B2_0_0_{CH_IN_YAML}.tiff", np.zeros((4, 4, 3), dtype=np.uint8))
...
def test_a_file_and_an_empty_timepoint_folder_are_refused_by_name(tmp_path):
    f = tmp_path / "x.tiff"
    f.write_bytes(b"")
...
def test_opening_an_acquisition_lists_the_timepoint_folder_exactly_once(squid_dataset,
                                                                       monkeypatch):
    """Counts directory listings rather than timing them, so a reintroduced second scan fails this on any machine, unlike a timing-based benchmark."""
...
def test_the_index_seed_is_used_once_and_a_later_rebuild_re_reads_disk(squid_dataset):
    """A reader built with a seeded listing drops the seed after one use — keeping it would serve a stale listing after the folder changes underneath it."""
    root, _arrays = squid_dataset
    reader = open_reader(root)
