...
def _shape_dims(viewer, nsteps):
    """Shape the REAL ``ViewerModel.dims`` to *nsteps* — the pane's model is real now, and its ``dims`` attribute is frozen, so the shape is CONFIGURED"""
...
def test_double_clicking_the_plate_opens_a_window_on_that_region_and_moves_the_red_frame(
        qapp, napari_pane_stub, squid_dataset):
    """The other direction: plate and navigation must move together both ways, now via an independent window rather than the root's own slider."""
    root, _ = squid_dataset
    win = V.PlateWindow(None)
    win.ingest(str(root))
    regions = list(win._order)
...
def test_a_fresh_plate_shows_a_region_the_user_did_not_open_and_keeps_one_copy_of_it(
        qapp, squid_dataset):
    """``_mosaic_region`` is a view over the cursor, not a field that can drift behind its back."""
    root, _ = squid_dataset
    win = V.PlateWindow(None)
    win.ingest(str(root))
...
@pytest.mark.parametrize("nsteps", [(512, 512), (1, 512, 512)])
def test_focus_reports_when_no_z_slider_could_be_moved(qapp, napari_pane_stub,
                                                       squid_dataset, nsteps):
    """A 2D layer, or a one-plane stack, has no z axis to move; driving step[0] would move Y and still announce success."""
    root, _ = squid_dataset
    win = V.PlateWindow(None)
    win.ingest(str(root))
    w = _open_window(win, ["B3"])
    _shape_dims(w._napari_viewer(), nsteps)

    w._on_reference_plane(4, "")

    assert "no z slider could be moved" in " ".join(w._pane.said), w._pane.said
    assert w._napari_viewer().dims.current_step == tuple(0 for _ in nsteps), (
        "y was driven instead of z")
    shutdown_plate_window(qapp, win)


def test_the_answer_is_clamped_to_the_stack_this_window_is_showing(
        qapp, napari_pane_stub, squid_dataset):
    """A z index past the stack's end is a crash or a no-op depending on napari version; clamp instead."""
    root, _ = squid_dataset
    win = V.PlateWindow(None)
    win.ingest(str(root))
    w = _open_window(win, ["B3"])
