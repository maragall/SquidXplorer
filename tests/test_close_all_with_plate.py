...
(``app.property("_squidmip_test")``) because a modal ``QMessageBox`` in a suite hangs with nobody
...
from squidmip import _prefs                                          # noqa: E402
from squidmip._viewer import PlateWindow                             # noqa: E402
...
def test_a_preference_that_was_never_set_reads_the_callers_default(tmp_path, monkeypatch):
    monkeypatch.setenv("SQUIDMIP_PREFS", str(tmp_path / "prefs.json"))
...
def test_setting_a_preference_puts_it_on_disk_and_reads_back(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SQUIDMIP_PREFS", str(path))
...
def test_an_unwritable_location_is_reported_not_raised(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SQUIDMIP_PREFS", str(blocker / "prefs.json"))
