...
def test_operator_validator_accepts_region_operators_and_names_what_it_can_run(squid_dataset):
...
def test_help_lists_every_operators_declared_parameters():
    """Generated from the registry, and checked against the registry itself, not a hand-copied list of today's declarations — a copy can't notice the"""
...
def test_a_fused_save_reports_off_its_own_manifest(squid_dataset, tmp_path):
    """The installer CI's exact path: a CLI stitch save lands the fused Squid OME-TIFF format, whose manifest carries no 'plate'/'levels'/'tiff' keys."""
...
def test_a_register_save_reports_without_a_writer_manifest(squid_dataset, tmp_path):
    """register's save artifact is its own stitched_ copy, so the runner hands back no writer manifest — the CLI must word that rather than crash (found by"""
    pytest.importorskip("tilefusion")
    root, _ = squid_dataset
    manifest = run(ProcessParameters(input_folder=str(root), output_folder=str(tmp_path),
                                     operator="register"))
...
def test_a_96_well_plate_is_not_refused_nor_is_a_format_override(squid_dataset, tmp_path,
                                                                 monkeypatch):
    root, _ = squid_dataset
    ...
    assert run(ProcessParameters(input_folder=str(root), output_folder=str(tmp_path / "o"),
                                 overwrite=True))["n_wells"] == 2
...
def test_rerun_refuses_to_write_over_a_finished_plate(squid_dataset, tmp_path):
    ...
    again = run(ProcessParameters(input_folder=str(root), output_folder=str(tmp_path),
                                  limit=1, overwrite=True))
    assert again["n_wells"] == 1


def test_an_incomplete_plate_is_refused_as_INPUT(squid_dataset, tmp_path):
    """A written plate is a legal input; the opener must refuse one the store itself marks incomplete, rather than silently projecting whatever wells happened to land."""
...
def test_the_summary_line_counts_fields_and_wells_each_against_its_own_total(squid_dataset,
                                                                            tmp_path, caplog):
    """fields and wells are two different units; the summary must report each against its own total rather than dividing one by the other."""
    import logging
