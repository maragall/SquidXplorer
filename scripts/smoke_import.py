...
MODULES = [
    ...
    # `squidxplorer._video`, `imageio` and `imageio_ffmpeg` are BACK, 2026-08-05, and so is the
    # feature. They were in this list from its first commit (03c5246), were dropped on 2026-07-31
    # when `squidxplorer/_video.py` was deleted by the other half of IMA-185 (c25d84d, "remove
    ...
    # visibly. Both packages are now declared -- `squidxplorer[video]`, pulled in by `[gui]` and
    # `[test]` -- so this gate is asserting something the project installs on purpose.
    "squidxplorer._video", "imageio", "imageio_ffmpeg",
    # The Qt binding is imported through qtpy, never by name, and squidxplorer/__init__ pins it to
