...
Open PowerShell in the SquidXplorer folder and run:

```powershell
powershell -ExecutionPolicy Bypass -File scripts\Setup-Windows.ps1
```

That builds a self-contained environment and puts a **SquidXplorer** icon on your Desktop.

To update later: `git pull` in the folder, then open the icon again. Nothing to reinstall.
...
1. **Drag the folder onto the Desktop icon.** It opens straight into that acquisition.
2. Open the icon, then **drop the folder onto the plate**.
3. Open the icon, then **File → Open acquisition folder**.
...
background subtraction, and nuclei detection. The ones backed by separate packages need those
installed.
