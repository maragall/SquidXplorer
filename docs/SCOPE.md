...
| **napari 3D is coarse-only** | napari drops to the COARSEST pyramid level whenever `ndisplay == 3` (`napari/layers/_scalar_field/_slice.py`: `if ndisplay == 3: level = len(self.data) - 1`). On the 10x set that is 128x107 - the "super ugly" 3D. Not fixable inside napari. The answer is the texture-bounded native volume with a crop (`_napari3d.py`), NOT AGAVE: AGAVE is cancelled (Julio, 2026-07-28), see `docs/VERSIONS.md`. |
...
| **Windows** | **BROKEN, confirmed.** FIXED on `main-window-review` (`d3b6a7a`). `acquire_gui_slot` did a bare `import fcntl` (now near `_viewer.py:7604`, the old `:7010` citation is stale), which does not exist on Windows and is reached from `showEvent` on every real launch, straight through the path the Desktop shortcut uses; only `GuiAlreadyOpen` was caught, so `ModuleNotFoundError` propagated and the app died before a window appeared. Also `/usr/bin/odon` and `/usr/bin/time` hardcoded. No CI on Windows. |
...
| **`_viewer.py` is 7,668 lines** | A god object: plate, operators, workers, tabs, export, GUI slot, napari bridge. Grew from 2,081 lines in 199 commits (+268%), workers 4 to 9, plus a second 1,507-line window class in `_region_viewer.py`. The terminal DID come out (`_terminal.py`, `e1d6018`), and nothing else has. The cost is measured: `origin/ima-qt6` was abandoned because its payload sat in this file. Most bugs found this session were seams inside it that nothing forced apart. |
